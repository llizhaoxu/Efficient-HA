...
def get_mode_offsets(mode: str):
    ...
    if mode == "prev":
        ...
        return [ 1,  2]
    elif mode == "prev2_next2":
        return [-2, -1, 1, 2]
    elif mode == "prev2_prev_next":
        return [-2, -1, 1]
    else:
...
########################################
# 3. 核心：在指定层范围内，按模式计算 JSD
########################################
def compute_layer_jsd_with_mode(
    probs: torch.Tensor,
    layer_min: int,
    layer_max: int,
    mode: str = "prev_next",
    ...
    """
    probs: (L, 1, T, V) 或 (L, T, V)
    layer_min, layer_max: 指定层范围 (0-based)，最终输出的层必须在范围内
    mode: 使用哪种邻居模式
    返回:
        jsd_mean: (K, T)  # K = 有效层数
        valid_layers: (K,)  # K 个真正被选中的层 index
    """

    # -------- flatten batch 维度 -------
    if probs.dim() == 4:
        probs_flat = probs.squeeze(1)   # (L, T, V)
    else:
        probs_flat = probs              # (L, T, V)

    L = probs_flat.size(0)
    offsets = get_mode_offsets(mode)

    # ---------- 选择能满足条件的有效层 ----------
    idx = torch.arange(L, device=probs.device)

    # 限制在基本范围
    valid = (idx >= layer_min) & (idx <= layer_max)

    # 确保 offset 后的层也在两个范围内
    for off in offsets:
        neigh = idx + off
        valid &= (neigh >= 0) & (neigh < L)
        valid &= (neigh >= layer_min) & (neigh <= layer_max)

    valid_layers = idx[valid]
    if valid_layers.numel() == 0:
        return None, torch.empty(0, dtype=torch.long, device=probs.device)

    # ----------- 计算每个 offset 的 JSD（矩阵化）------------
    jsd_list = []
    for off in offsets:
        p = probs_flat[valid_layers]         # (K, T, V)
        q = probs_flat[valid_layers + off]   # (K, T, V)
        jsd_val = jsd_vec(p, q)              # (K, T)
        jsd_list.append(jsd_val)

    # 堆叠取平均
    jsd_stack = torch.stack(jsd_list, dim=0)   # (num_offsets, K, T)
    jsd_mean = jsd_stack.mean(dim=0)           # (K, T)

    return jsd_mean, valid_layers

def ours_greedy_search(
    self,
    ...
    **model_kwargs,
    ...
    while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
        ...
        logits_dict = {}
        logits = torch.stack(outputs.hidden_states)[:,  :,-1,  :] 
# shape: (L, 1, T, H)   # L = 总层数，T = seq_len
        logits = lm_head(logits)      # (L, 1, T, V)
        token_hidden_states  = torch.softmax(logits, dim=-1).detach().cpu()    # (L, 1, T, V)   
        jsd_mean, valid_layers = compute_layer_jsd_with_mode(
            token_hidden_states,
            layer_min=30,
            layer_max=len(outputs.hidden_states),
            mode="prev_next",
        )
        ...
        target_layers = valid_layers[torch.argmin(jsd_mean,dim=0)]  # (T,)
        selected_premature_layer_idx = target_layers[-1].item()    
        selected_premature_layer_logits=logits[selected_premature_layer_idx]
        premature_max_probs=token_hidden_states[selected_premature_layer_idx,:,candidate_tokens_ids.cpu()].squeeze(dim=1).max().item()
...
def evolve_ours_greedy():

    # sample is now a protected function in the latest Transformers library
    transformers.generation.utils.GenerationMixin._sample = ours_greedy_search
