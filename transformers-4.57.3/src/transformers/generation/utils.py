...
GENERATION_MODES_MAPPING = {
    ...
    GenerationMode.OURS: "_ours",
    GenerationMode.SAVER: "_saver",
    GenerationMode.DECO: "_deco",
    # Deprecated methods
...
class GenerationMixin(ContinuousMixin):
    ...
    def _ours(
        ...
        **model_kwargs,
        ...
        lm_head=generation_config.lm_head
        norm=generation_config.norm
        pad_token_id = generation_config._pad_token_tensor
        output_attentions = generation_config.output_attentions
        output_hidden_states = generation_config.output_hidden_states
        ...
        while self._has_unfinished_sequences(
            ...
            del outputs,logits,probs,layer_jsd_mean
    ...
    def _saver(
            ...
            **model_kwargs,
            ...
            ours_layers=generation_config.early_exit_layers
            ours_alpha=generation_config.saver_alpha
            ours_threshold_top_k_small=generation_config.saver_threshold_top_k_small
            ours_threshold_top_p=generation_config.saver_threshold_top_p
            ours_threshold_top_k=generation_config.saver_threshold_top_k
            ...
            if isinstance(ours_layers, str) and ours_layers == "low":
            ...
            elif isinstance(ours_layers, str) and ours_layers == "high":
                candidate_premature_layers = (
            ...
            elif isinstance(ours_layers, list):
                candidate_premature_layers = [i for i in ours_layers if i < final_layer]
            else:
            ...
            key_position=generation_config.position
            if key_position is None:
                image_token_index = input_ids.tolist()[0].index(-200)
                image_end=24*24+image_token_index
            else:
                image_token_index=key_position['image_start']
                image_end=key_position['image_end']
            # 4. run the generation loop

            first_logits_dict = {}
            first=True
            temp=torch.zeros(len(candidate_premature_layers)).cuda()
            gamma_id=[]
            while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
                ...
                model_outputs = self(**model_inputs, return_dict=True, output_hidden_states=True)
                ...
                logits_dict = {}
                return_map_dict = []
                logits_warper = TopKLogitsWarper(top_k=ours_threshold_top_k_small, filter_value=float("-inf"))
                if first:
                    for _, early_exit_layer in enumerate(candidate_premature_layers):
                        like_final_outputs = model_outputs.hidden_states[early_exit_layer]
                        logits = lm_head(like_final_outputs)
                        logits=F.log_softmax(logits, dim=-1)
                        logits = logits_warper(input_ids, logits)
                        logits=F.softmax(logits, dim=-1)
                        logits=logits[:,image_token_index:image_end]
                        first_logits_dict[early_exit_layer] = logits
                    if idx!=None:
                        temp_conf=torch.stack([first_logits_dict[i] for i in candidate_premature_layers], dim=0)
                        temp_conf=temp_conf.sum(1)
                        temp_conf=temp_conf.sum(1)
                        temp_conf=temp_conf[:,idx]
                        temp_conf=temp_conf.sum(1)
                    first=False
                ...
                logits_dict[len(model_outputs.hidden_states)] = lm_head(model_outputs.hidden_states[-1])

                last_layer_tokens_logits = model_outputs.logits[:, -1, :]
                last_layer_tokens_probs = nn.functional.softmax(last_layer_tokens_logits, dim=-1)
                multi_candidate_tokens_probs, multi_candidate_tokens_ids = torch.topk(last_layer_tokens_probs, dim=-1, k=ours_threshold_top_k)
                ...
                for seq_idx in range(multi_candidate_tokens_cumulative_probs.size(0)):
                    candidate_tokens_indices = torch.searchsorted(multi_candidate_tokens_cumulative_probs[seq_idx,:], ours_threshold_top_p, right=False)
                    candidate_tokens_cutoff_idx = torch.min(candidate_tokens_indices + 1, torch.tensor(ours_threshold_top_k))
                    candidate_tokens_ids = multi_candidate_tokens_ids[seq_idx,:candidate_tokens_cutoff_idx]
                    image_conf_layers=torch.stack([first_logits_dict[i][seq_idx, :, :] for i in candidate_premature_layers], dim=0)
                    candidate_tokens_image_conf = image_conf_layers.sum(1)[:,candidate_tokens_ids]# [10 layers, 10 candidate tokens]
                    stacked_early_exit_layers = torch.stack([logits_dict[i][seq_idx, -1, :] for i in candidate_premature_layers], dim=0)
                    ...
                    max_candidate_tokens_idx_image_conf = torch.argmax(candidate_tokens_image_conf)
                    temp=temp+candidate_tokens_image_conf.sum(1)
                    max_candidate_tokens_idx = torch.argmax(candidate_tokens_early_exit_probs)

                    premature_max_probs_image_conf = candidate_tokens_image_conf.max().item()
                    target_layers = max_candidate_tokens_idx // candidate_tokens_early_exit_probs.size(1) 
                    target_image_conf = max_candidate_tokens_idx_image_conf // candidate_tokens_early_exit_probs.size(1)
                    
                        

                    selected_premature_layer_idx = candidate_premature_layers[target_layers.item()]
                    selected_premature_layer_idx_image_conf = candidate_premature_layers[target_image_conf.item()]

                    selected_premature_layer_logits = logits_dict[selected_premature_layer_idx][seq_idx, -1, :] # [1, vocab_size]
                    selected_premature_layer_logits_image_conf = logits_dict[selected_premature_layer_idx_image_conf][seq_idx, -1, :] # [1, vocab_size]
                    indices_to_remove_image_conf = torch.ones_like(selected_premature_layer_logits_image_conf)
                    indices_to_remove_image_conf[candidate_tokens_ids] = 0
                    indices_to_remove_image_conf = indices_to_remove_image_conf.bool()

                    indices_to_remove = torch.ones_like(selected_premature_layer_logits)

                    indices_to_remove[candidate_tokens_ids] = 0
                    indices_to_remove = indices_to_remove.bool()
                    next_token_logits = model_outputs.logits[seq_idx, -1, :]
                    # final_token_logits = next_token_logits + 0.6 * premature_max_probs * selected_premature_layer_logits
                    final_token_logits = next_token_logits + ours_alpha * premature_max_probs_image_conf * selected_premature_layer_logits_image_conf
                    final_token_logits = final_token_logits.masked_fill(indices_to_remove_image_conf, -float("Inf"))
                    tensor_list.append(final_token_logits)
                    gamma_list.append(premature_max_probs_image_conf)
                next_token_logits = torch.stack(tensor_list,dim=0)
                gamma_id.append(gamma_list)

                logits = next_token_logits.clone().float()  # Clone is needed to avoid keeping a hanging ref
